    ...
    <div className="border border-slate-300 bg-white text-xs">
      ...
      <div className="grid grid-cols-7 auto-rows-fr">
        {gridCells.map((date, idx) => {
          ...
          if (publicHoliday) {
            bgClass = 'bg-[#ffcc00] font-bold text-black'; // Public holiday yellow/orange
            tooltip = publicHoliday.title;
          } else if (schoolHoliday) {
            bgClass = 'bg-[#89d6e8] text-black'; // School holiday cyan/blue
            tooltip = schoolHoliday.term;
          } else if (date.getDay() === 0 || date.getDay() === 6) {
             bgClass = 'bg-slate-50 text-slate-500'; // Weekend light gray
          }
